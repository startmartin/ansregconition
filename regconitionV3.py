...
class RecognitionV3:
    ...
    def __init__(self, config):
        # self.image = cv2.imread("/Users/vito/Downloads/模拟联考题卡/英语/Scan Data from FX-1C7D22314B77/20230311183910-0001.tif")
    ...
    def transformation(self, image):
        ...
        # color_image = cv2.cvtColor(morph, cv2.COLOR_GRAY2BGR)
        # for c in cnts:
        #     self.draw_contours(color_image, c)
        #     # cv2.drawContours(color_image, c, -1, (0, 0, 255), 2)
        # plt.figure("寻找定位坐标点")
        # plt.imshow(imutils.opencv2matplotlib(color_image))
        # plt.show()
        exit(0)
